...
- Nonce-issuing policy (when/whether to require one) is out of scope (§8); for replay, servers "can store" each proof's `jti` for its acceptance window to prevent multiple uses — optional practice, not an absolute requirement, and strict single-use checking "may not always be feasible in practice" (§11.1).
